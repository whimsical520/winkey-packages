...
class WkMiniProgram {
    ...
    try {
      ...
      const handleFile = (entryPath, outputPath, type?: LogType) => {
        ...
        if (filename.includes('.ts')) {
          ...
          this.log(`[ts-js] [${path.resolve(path.resolve(outputPath, '../'), match[1] + '.js')}]`, type || LogType.Success)
        } else if (/(.*).wxml/.test(filename)) {
          fs.copyFileSync(entryPath, outputPath)
  
          this.log(`[wxml-wxml] [${path.resolve(path.resolve(outputPath, '../'), filename)}]`, type || LogType.Correct)
        } else if (/(.*).json/.test(filename)) {
          fs.copyFileSync(entryPath, outputPath)
  
          this.log(`[json-json] [${path.resolve(path.resolve(outputPath, '../'), filename)}]`, type || LogType.Correct)
          ...
          esbuild.build({
            entryPoints: [entryPath],
            outfile: path.resolve(path.resolve(outputPath, '../'),  match[1] + '.wxss'),
            bundle: true,
            plugins: [lessLoader()]
          }).catch((err) => {
            logger(LogType.Error, err)
          ...
          this.log(`[less-wxss] [${path.resolve(path.resolve(outputPath, '../'),  match[1] + '.wxss')}]`, type || LogType.Success)
        } 
      }
      ...
      if (type === ExecType.Dev) {
        ...
        watcher.on('all', (event, filepath) => {
          const outputPath = path.join(__dirname, 'output', filepath.split('src')[1]) 
          switch(event) {
            case 'change':
  ...
  private getProjectJson() {
    ...
    if (fs.existsSync('project.config.json')) {
      fs.copyFileSync('project.config.json', path.resolve(this.baseOutputPath, 'project.config.json'))
      return
    }
  }
  ...
  private getNodeModulesDataByDependencies() {
    ...
    for (const key in dependencies) {
      ...
      if (subPackageJSON.main) {
        ...
        logger(LogType.Correct, `[${moment().format('HH:mm:ss')}] [npm-npm] [${resultPath}] copy`)
      }
