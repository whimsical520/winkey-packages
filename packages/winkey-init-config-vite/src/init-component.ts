...
export const initComponent = (localConfig: WinkeyProjectConfig, viteConfig?: ViteConfigStruct) => {
  /** 包名 */
  ...
  const config: any = {
    ...
    build: {
      ...
      rollupOptions: {
        // 确保外部化处理那些你不想打包进库的依赖
        external: ['react', 'react-dom', ...(localConfig?.build?.rollupOptions?.external || [])],
        output: {
  ...
  /** 合并plugin */
  config.plugins = Object.assign(config.plugins as any[], localConfig.plugins || [])
  /** 合并css */
  ...
  if (isBuild) {
    ...
    config.plugins = [
      ...
      typescript({
        ...
        exclude: [path.resolve('node_modules/**'), path.resolve('winkey.config.ts')],
  ...
  config.plugins = [
    ...
    createHtmlPlugin({
      inject: {
        data: {
          ...env,
          devServerToolScript: localConfig.winkeyTool && !isBuild
            ? `<script type="module" src="/winkey_tool/js/index.js"></script>`
            : '',
          devServerToolDom: localConfig.winkeyTool && !isBuild
              ? `<div id="winkey_tool"></div>`
              : '',
          devServerToolCss: localConfig.winkeyTool && !isBuild
